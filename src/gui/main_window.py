...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        eye_icon.setPixmap(pix.scaled(18, 18, Qt.KeepAspectRatio, Qt.SmoothTransformation) if not pix.isNull() else QPixmap())
        if pix.isNull():
            eye_icon.setText("👁")
        text_label = QLabel("Gefäße"); text_label.setObjectName("MutedLabel")
        ...
        hl = QHBoxLayout(footer_container); hl.setContentsMargins(0, 0, 0, 0); hl.setSpacing(0)
        footer_label = QLabel(f"© Franziska Krauß 2025", footer_container)
        footer_label.setObjectName("FooterLabel")
        footer_label.setAlignment(Qt.AlignCenter)
        hl.addStretch(1); hl.addWidget(footer_label, 0); hl.addStretch(1)
        ...
        trans_label = QLabel(" Transparenz"); trans_label.setObjectName("MutedLabel")
        slider_row.addWidget(trans_label); slider_row.addWidget(self.overlay_slider)
    ...
    def on_model_ready(self):
        ...
        # Start the video now (pending user choice or auto)
    ...
    def start_video_thread(self, src: str | int = "auto"):
        ...
        # Connect signals (no double feeding!)
    ...
    def update_video_frame(self, qimg: QImage):
        ...
        try:
            ...
            pill.style().unpolish(pill); pill.style().polish(pill); pill.update()
        except Exception:
            pass
    ...
    # Screenshot: compose frame + overlay with opacity
    def on_screenshot_clicked(self):
        ...
        if self._last_overlay_qimg is not None and self.vessel_toggle.isChecked():
            ...
            painter.drawImage(0, 0, self._last_overlay_qimg.scaled(base.size(), Qt.KeepAspectRatioByExpanding, Qt.SmoothTransformation))
        painter.end()

        start_dir = getattr(self, "_last_save_dir", os.getcwd())
        path, _ = QFileDialog.getSaveFileName(
            self, "Screenshot speichern", os.path.join(start_dir, "screenshot.png"),
        ...
        result.save(path)
        self.footer.showMessage(f"Screenshot gespeichert: {os.path.basename(path)}", 4000)
